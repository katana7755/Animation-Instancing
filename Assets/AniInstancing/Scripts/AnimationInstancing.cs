...
namespace AnimationInstancing
{
    ...
    public class AnimationInstancing : MonoBehaviour
    {
        ...
        */
        ...
        [Header("[Additional data for sample project]")]
    ...
    // [Unity] Replace AnimationInstancing GameObjects with normal class objects +++++
    public class AnimationInstancingObject
    {
        ...
        public AnimationInstancing.LodInfo[] lodInfo
        {
            get;
            private set;
        }
        ...
        public void Start(AnimationInstancing instancingPrefab, System.Action<AnimationEvent> aniEventListener)
        {
            ...
            prefab = instancingPrefab.gameObject;
            prototype = instancingPrefab.prototype;
            localToWorldMatrix = Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(Vector3.zero), Vector3.one);
            localPosition = Vector3.zero;
            localRotation = Quaternion.Euler(Vector3.zero);
            position = Vector3.zero;
            boundingSpere = new BoundingSphere(new Vector3(0, 0, 0), 1.0f);
            layer = instancingPrefab.gameObject.layer;
            shadowCastingMode = instancingPrefab.shadowCastingMode;
            receiveShadow = instancingPrefab.receiveShadow;
            applyRootMotion = instancingPrefab.applyRootMotion;
            this.aniEventListener = aniEventListener;
            aniIndex = -1;
            preAniIndex = -1;
            aniTextureIndex = -1;
            // [Unity] Fix test functionality(RandomCharacters) +++++
            randomCharacterData = instancingPrefab.randomCharacterData.Clone();
            // +++++

            LODGroup lod = instancingPrefab.GetComponent<LODGroup>();
            if (lod != null)
            {
                lodInfo = new AnimationInstancing.LodInfo[lod.lodCount];
                LOD[] lods = lod.GetLODs();
                for (int i = 0; i != lods.Length; ++i)
                {
                    if (lods[i].renderers == null)
                    ...
                    AnimationInstancing.LodInfo info = new AnimationInstancing.LodInfo();
                    info.lodLevel = i;
                    info.vertexCacheList = new AnimationInstancingMgr.VertexCache[lods[i].renderers.Length];
                    info.materialBlockList = new AnimationInstancingMgr.MaterialBlock[info.vertexCacheList.Length];
                    List<SkinnedMeshRenderer> listSkinnedMeshRenderer = new List<SkinnedMeshRenderer>();
            ...
            {
                lodInfo = new AnimationInstancing.LodInfo[1];
                AnimationInstancing.LodInfo info = new AnimationInstancing.LodInfo();
                info.lodLevel = 0;
                info.skinnedMeshRenderer = instancingPrefab.GetComponentsInChildren<SkinnedMeshRenderer>();
                info.meshRenderer = instancingPrefab.GetComponentsInChildren<MeshRenderer>();
                info.meshFilter = instancingPrefab.GetComponentsInChildren<MeshFilter>();
                info.vertexCacheList = new AnimationInstancingMgr.VertexCache[info.skinnedMeshRenderer.Length + info.meshRenderer.Length];
                info.materialBlockList = new AnimationInstancingMgr.MaterialBlock[info.vertexCacheList.Length];
                lodInfo[0] = info;
            ...
            CalcBoundingSphere();
            AnimationInstancingMgr.Instance.AddBoundingSphere(this);
            AnimationInstancingMgr.Instance.AddInstance(this);
        }
        ...
        public void UpdateLod(Vector3 cameraPosition)
        {
            ...
            if (lodFrequencyCount > lodCalculateFrequency)
            {
                float sqrLength = (cameraPosition - position).sqrMagnitude;
                if (sqrLength < 50.0f)
                    lodLevel = 0;
                else if (sqrLength < 500.0f)
                    lodLevel = 1;
        ...
        public bool IsLoop() { return wrapMode == WrapMode.Loop; }
        public bool IsPause() { return speedParameter == 0.0f; }

        public void SetPositionAndRotation(Vector3 localPosition, Quaternion localRotation)
        {
            this.localPosition = localPosition;
            this.localRotation = localRotation;
            this.localToWorldMatrix = Matrix4x4.TRS(this.localPosition, this.localRotation, Vector3.one);
        ...
        {
            Bounds bound = new Bounds(new Vector3(0, 0, 0), new Vector3(1, 1, 1));
            AnimationInstancing.LodInfo info = lodInfo[0];
            for (int i = 0; i != info.meshRenderer.Length; ++i)
            {
                MeshRenderer meshRenderer = info.meshRenderer[i];
                bound.Encapsulate(meshRenderer.bounds);
        ...
        private float speedParameter = 1.0f, cacheParameter = 1.0f;
